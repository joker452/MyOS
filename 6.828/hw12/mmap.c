...
handle_sigsegv(int sig, siginfo_t *si, void *ctx)
{
  ...
  static double *mapped_page = NULL;
  // replace these three lines with your implementation
  uintptr_t fault_addr = (uintptr_t)si->si_addr;
  // unmap previous page
  if (mapped_page && munmap(mapped_page, page_size) == -1)
  {
    fprintf(stderr, "Couldn't munmap() region for sqrt table in handle_sigsegv; %s\n",
            strerror(errno));
    exit(EXIT_FAILURE);
  }

  mapped_page = mmap((void *) align_down(fault_addr, page_size), page_size, PROT_READ | PROT_WRITE,
               MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);

  if (mapped_page == MAP_FAILED)
  {
    fprintf(stderr, "Couldn't mmap() region for sqrt table in handle_sigsegv; %s\n",
            strerror(errno));
    exit(EXIT_FAILURE);
  }

  // because we put the first sqrt in this range in mapped_page[0], so its difference with sqrts should be the start
  calculate_sqrts(mapped_page, ((double *) mapped_page) - sqrts, page_size / sizeof(double));
}
